...
const ZH_CN: [&str; KEY_COUNT] = [
    "确认", "新建", "全选", "启动选中项", "搜索...", "条目", "最近", "暂无条目",
    "点击「新建」添加第一个启动条目", "新建条目", "无匹配结果", "换个关键词试试", "编辑", "删除",
    "上移", "下移", "主题：自动（跟随系统）— 点击循环：自动 → 深色 → 浅色",
    "语言：自动（跟随系统）— 点击循环：自动 → 中文 → English", "自动", "中文", "EN", "名称", "目录",
    "命令", "终端（可选）", "必填", "例如 D:\\projects\\demo", "例如 pwsh、cmd、powershell",
    "启动前确认", "新建条目", "编辑条目", "保存", "取消", "删除", "目录存在", "目录不存在",
    "名称不能为空", "命令不能为空", "确认启动", "启动", "删除条目", "删除「{0}」？\n此操作无法撤销。",
    "确认 {0} 个启动", "全部启动", "名称：{0}", "命令：{0}", "目录：{0}", "已添加", "已更新",
    "已删除", "已启动：{0}", "已启动 {0} 个条目", "已启动 {0}/{1} 个条目（{2} 个失败）",
...
const EN_US: [&str; KEY_COUNT] = [
    "Confirm", "New", "Select All", "Launch Selected", "Search...", "ITEMS", "RECENT",
    "No items yet", "Click + New to add your first launch item", "New Item", "No matches",
    "Try a different search query", "Edit", "Delete", "Move Up", "Move Down",
    "Theme: auto (follow system) — click cycles: auto → dark → light",
    "Language: auto (follow system) — click cycles: auto → Chinese → English", "Auto", "中文",
    "EN", "Name", "Directory", "Command", "Terminal (optional)", "Required", "e.g. D:\\projects\\demo",
    "e.g. pwsh, cmd, powershell", "Confirm before launch", "New Item", "Edit Item", "Save",
    "Cancel", "Delete", "Directory exists", "Directory does not exist", "Name is required",
    "Command is required", "Confirm Launch", "Launch", "Delete Item",
    "Delete '{0}'?\nThis cannot be undone.", "Confirm {0} launches", "Launch All", "Name: {0}",
    "Command: {0}", "Directory: {0}", "Added", "Updated", "Deleted", "Launched: {0}",
    "Launched {0} items", "Launched {0} of {1} items ({2} failed)", "Config error: {0}",
...
mod tests {
    ...
    fn effective_auto_follows_system() {
        assert_eq!(AppLanguage::ZhCn, effective(AppLanguage::Auto, AppLanguage::ZhCn));
        assert_eq!(AppLanguage::EnUs, effective(AppLanguage::Auto, AppLanguage::EnUs));
        assert_eq!(AppLanguage::ZhCn, effective(AppLanguage::ZhCn, AppLanguage::EnUs));
        assert_eq!(AppLanguage::EnUs, effective(AppLanguage::EnUs, AppLanguage::ZhCn));
    }
    ...
    fn format_fills_placeholders() {
        let zh = format(LanguageKey::DialogDeleteItemMessage, AppLanguage::ZhCn, &["snow"]);
