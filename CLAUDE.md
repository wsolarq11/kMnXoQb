...
2. 端口接口定义在 `Core/Ports`，由 Infrastructure 实现，DI 注入（`App.xaml.cs` 的 `ConfigureServices` 全部按接口注册——**DI 按具体类型解析会失败**，Attach 类方法用 `(Concrete)_services.GetRequiredService<IInterface>()`）。
...
- **unpackaged FolderPicker 必须 `WinRT.Interop.InitializeWithWindow.Initialize(picker, hwnd)`**，否则抛异常；XamlRoot 在 Activate 之后才可用（DialogService 懒解析）。
